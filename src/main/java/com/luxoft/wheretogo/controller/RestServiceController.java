...
//				request.getSession().setAttribute("user", user);
	...
	public UserInfo user(HttpServletRequest request, Principal principal) {
		...
		return new UserInfo(user.getRole(), user.getEmail(), user.getFirstName(), user.getLastName(), user.isActive(), user.getPicture(), user.getId(),user.getDescription(),user.getPhoneNumber());
	}
